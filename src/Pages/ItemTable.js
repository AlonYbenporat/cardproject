...
function ItemTable() {
  ...
  useEffect(() => {
    const fetchItems = async () => {
      try {
        ...
        const itemsWithLikes = await Promise.all(filteredItems.map(async (item) => {
          const validUserIds = item.likes.filter(userId => userId);
          if (validUserIds.length > 0) {
            const likedByUsers = await Promise.all(validUserIds.map(async (userId) => {
              try {
                const userResponse = await axios.get(`${BaseUrlusers}${userId}`, {
        ...
        const totalLikesCount = itemsWithLikes.reduce((total, item) => total + item.likes.length, 0);
  ...
  const handleSort = (key) => {
    const sorted = [...items].sort((a, b) => {
      if (key === 'likes') {
        return sortDirection === 'asc' ? b[key].length - a[key].length : a[key].length - b[key].length;
      } else {
  ...
  return (
    ...
    <select  className="text-center" value={selectedDays} onChange={(e) => setSelectedDays(parseInt(e.target.value))}>
    ...
    </select><i class="bi bi-funnel"></i></h3>
    
      <div  className="size">
      <i class="bi bi-filetype-csv m-2" onClick={() => handleExportCSV(filteredItems)} ></i>
    ...
    <thead >
        <tr >
            ...
            <th  scope="col"  
            onClick={() => handleSort('subtitle')}>Subtitle{' '}
            {sortDirection === 'asc' ? (
                <i className="bi bi-sort-alpha-down"></i>
            ) : (
                <i className="bi bi-sort-alpha-up"></i>
            ...
            <th  scope="col" 
            onClick={() => handleSort('web')}>Website{' '}
            {sortDirection === 'asc' ? (
                <i className="bi bi-sort-alpha-down"></i>
            ) : (
                <i className="bi bi-sort-alpha-up"></i>
            ...
            <th  scope="col"  
            onClick={() => handleSort('address.state')}>State{' '}
            {sortDirection === 'asc' ? (
            <i className="bi bi-sort-alpha-down"></i>
            ) : (
            <i className="bi bi-sort-alpha-up"></i>
            ...
            <th  scope="col"
            onClick={() => handleSort('address.country')}> Country{' '}
            {sortDirection === 'asc' ? (
            <i className="bi bi-sort-alpha-down"></i>
            ) : (
            <i className="bi bi-sort-alpha-up"></i>
            ...
            <th  scope="col" 
            onClick={() => handleSort('address.city')}>City{' '}
            {sortDirection === 'asc' ? (
            <i className="bi bi-sort-alpha-down"></i>
            ) : (
            <i className="bi bi-sort-alpha-up"></i>
            ...
            <th  scope="col" 
            onClick={() => handleSort('address.street')}>Street{' '}
            {sortDirection === 'asc' ? (
            <i className="bi bi-sort-alpha-down"></i>
            ) : (
            <i className="bi bi-sort-alpha-up"></i>
            ...
            <th  scope="col"  
            onClick={() => handleSort('address.zip')}>Zip{' '}
            {sortDirection === 'asc' ? (
                <i className="bi bi-sort-alpha-down"></i>
            ) : (
                <i className="bi bi-sort-alpha-up"></i>
        ...
        <tbody className={`text-${reversedTheme}`}>
          {filteredItems.map((item, index) => (
            <tr key={item._id}>
              ...
              <td>
                <i class="bi bi-trash"onClick={() => handleDelete(item._id)}></i>
              </td>
              <td 
              ...
              <td>{item.title}</td>
              <td>{item.subtitle.length > 10 ? `${item.subtitle.slice(0, 10)}...` : item.subtitle}</td>
              <td 
                title={item.web} 
                onClick={() => window.open(item.web, '_blank')}>
                {item.web.length > 10 ? `${item.web.slice(0, 10)}...` : item.web}
                </td>
              <td>{item.email}</td>

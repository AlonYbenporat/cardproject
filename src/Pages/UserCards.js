...
function UserCards() {
    ...
    useEffect(() => {
        ...
        .then((response) => {
            const userItems = response.data.filter(item => item.user_id === userId)
            .map(item => ({
                ...item,
                ...
                <div className="row">
                    ...
                    {filteredItems.map((item, index) => (
                        <div key={item.id} className="col-md-4 mb-5">
                            <div className={`card bg-${theme} `} onClick={() => handleCardClick(item)} style={{ width: '300px', height: '500px' }}>
                            <button  style={{borderColor: '#FFFFFF'}}  onClick={() => handleLike(item._id) }>
                                ...
                                )} <span style={{ marginLeft: '5px', fontSize: '30px' }}>{item.likes.length}</span> 
                                </button>

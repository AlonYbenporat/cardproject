...
function AddItems() {
  ...
  const handleSubmit = async (e) => {
      ...
      try {
        ...
        if (error.response && error.response.status === 400) {
          if (error.response.data === 'Joi Error: card "phone" mast be a valid phone number') {
            setFailureMessage('Phone number is invalid');
          } else if (error.response.data === 'Joi Error: card "mail" mast be a valid mail') {
            setFailureMessage('Email is invalid');
          } else if (error.response.data === 'Joi Error: "title" length must be at least 2 characters long') {
            setFailureMessage(' Your title length is to short');
          } else if (error.response.data === 'Joi Error: "subtitle" length must be at least 2 characters long') {
            setFailureMessage(' Your subtitle length is to short');
          } else if (error.response.data === 'Joi Error: card "web" mast be a valid url') {
            setFailureMessage(' Website addrees inivalid');
          } else if (error.response.data === 'Joi Error: "address.country" length must be at least 2 characters long') {
            setFailureMessage(' country is to short');
          } else if (error.response.data === 'Joi Error: "address.city" length must be at least 2 characters long') {
            setFailureMessage(' city is to short');
          } else if (error.response.data === 'Joi Error: "address.street" length must be at least 2 characters long') {
            setFailureMessage('Street name is to short');
          } else if (error.response.data === 'Joi Error: "description" length must be at least 2 characters long') {
  ...
  return (
    ...
    <h1 className='text-center'>Just added Cards</h1>
<div className="container">
    <div className="row">
        {addedItems.map((item) => (
            <div key={item._id} className="col-md-4 mb-4">
                <div className="card">
                    <div className="card-body">
                        ...
                        <p className="card-text">{item.address.street}, {item.address.houseNumber}, {item.address.city}, {item.address.state}, {item.address.country}, {item.address.zip}</p>
                        <img src={item.image.url} alt={item.image.alt} className="card-img-top" />
                        <div className='text-center'>
                            <button className="btn btn-primary m-4" onClick={() => handleEdit(item)}>Edit</button>
                            <button className="btn btn-danger m-4" onClick={() => handleDelete(item._id)}>Delete</button>
                        </div>
                    </div>
                </div>
            </div>

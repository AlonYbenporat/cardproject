...
import { generateUsersPdf, handleExportCSV, handlePrint } from '../HelperFunctions/tableUtils';
...
function UserTable() {
  ...
  const handleEdit = async (userId) => {
    try {
        ...
        const updateResponse = await axios.patch(`${BaseUrlusers}${userId}`, userData, {
            headers: {
                'x-auth-token': storedToken
            }
        });
        if (updateResponse.status === 200) {
            ...
            const businessStatus = updatedUserData.isBusiness ? 'Business' : 'Regular';
            alert(`User "${updatedUserData.name.first} ${updatedUserData.name.last}" is now ${businessStatus}`);
            fetchUserData();
        } else {
          const updatedUserData = updateResponse.data;
            alert(`Failed to edit Buisness status to ${updatedUserData.name.first} ${updatedUserData.name.last}. Please try again later.`);
        }
    } catch (error) {
        console.error('Error editing user:', error);
        alert('An error occurred while editing Buisness user status. Please try again later.');
    }
};
...
const handleDelete = async (userId) => {
  try {
    ...
    const response = await axios.delete(`${BaseUrlusers}${userId}`, {
      headers: {
        'x-auth-token': token
      }
    });
    if (response.status === 200) {
      ...
      alert(`User ${name.first} ${name.last} deleted successfully`);
      fetchUserData();
    }} catch (error) {
      console.error('Error deleting user:', error);
      if (error.response && error.response.data  === 'Could not delete admin user') {
  ...
  for (const key in user) {
      if (Object.prototype.hasOwnProperty.call(user, key)) {
        if (typeof user[key] === 'string' && user[key].toLowerCase().includes(searchQuery.toLowerCase())) {
          return true;
        }
      }
    }
  ...
  return (
    <div className={`bg-${theme} text-${reversedTheme}`}>
      ...
      <div  className="size">
      <i class="bi bi-filetype-csv m-2" onClick={() => handleExportCSV(filteredUsers)} ></i>
      <i class="bi bi-file-pdf m-2" onClick={() => generateUsersPdf(filteredUsers)}></i>

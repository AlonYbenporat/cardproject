...
function NavBar() {
  ...
  return (
    <nav
      className={`navbar navbar-expand-sm navbar-${theme}`}
      style={{ backgroundColor: theme === "dark" ? "#1C2833" : "#12121dc6" }}>
          ...
          <Dropdown.Menu>
            {isLoggedIn ? (
              <>
                {isAdmin && (
                  <>
                    <Dropdown.Item onClick={() => navigate("/UsersTable")}>
                      Users Mangment
                    </Dropdown.Item>
                    <Dropdown.Item onClick={() => navigate("/ItemTable")}>
                      Card items Mangment{" "}
                    </Dropdown.Item>
                  </>
                )}{" "}
                {isBusiness && (
                  <Dropdown.Item onClick={handleAddItemsClick}>
                    AddCard
                  </Dropdown.Item>
                )}
                <Dropdown.Item onClick={() => navigate("/MyCards")}>
                  My cards
                </Dropdown.Item>
                <Dropdown.Item onClick={() => navigate("/MyLikes")}>
                  My Favorite
                </Dropdown.Item>
                <Dropdown.Item onClick={HandleEditUserDetails}>
                  Edit UserDetails
                </Dropdown.Item>
                <Dropdown.Item className="active" onClick={handleLogout}>
                  Logout
                </Dropdown.Item>
              </>
            ) : (
      ...
      <img
        ...
        style={{ width: "100px", height: "40px", margin: "15px" }}></img>
      <div className="form-check form-switch">
        ...
        <label
          ...
          {theme === "dark" ? (
            ...
            <i
              className={`bi bi-brightness-high m-1 text-${reversedTheme}`}></i>
          )}
        </label>
      </div>
      <HandleLogin
